/*
 * $Id: JGraphFacade.java,v 1.5 2010-12-01 17:23:05 david Exp $
...
public class JGraphFacade {
	...
	return DefaultGraphModel.getDescendants(model,
		DefaultGraphModel.getRoots(model));
    }
	...
	if (origin != null) {
	    translateCells(getAttributes().keySet(),
		    -rect.getX() + origin.getX(), -rect.getY() + origin.getY());
	} else if ((graph == null || !graph.isMoveBelowZero())
		&& (rect.getX() < 0 || rect.getY() < 0)) {
	    scale(getAttributes().keySet(), 1, 1,
		    (rect.getX() < 0) ? Math.abs(rect.getX()) : 0,
		    (rect.getY() < 0) ? Math.abs(rect.getY()) : 0);
	}
	...
	while (it.hasNext()) {
	    ...
	    if (location != null) {
		location.setLocation((location.getX() + dx) * scalex,
			(location.getY() + dy) * scaley);
	...
	for (int i = 0; i < v.length; i++)
	    setLocation(v[i], r + r * Math.sin(i * phi),
		    r + r * Math.cos(i * phi));
    }
	...
	if (isEdge(cell)) {
		...
		if (DefaultGraphModel.isVertex(model, child) && cell != child) {
			...
			// If visiblity information is available and the child
			// is
		...
		if (inset != 0) {
		    cellBounds.setFrame(cellBounds.getX() - inset,
			    cellBounds.getY() - inset, cellBounds.getWidth()
				    + inset * 2, cellBounds.getHeight() + inset
				    * 2);
		}
	...
	if (cell != null) {
		...
		if (moveGroups) {
			...
			if (graphLayoutCache != null) {
			    childVisible = graphLayoutCache
				    .isVisible(childCell);
			}
			if (DefaultGraphModel.isVertex(model, childCell)
				&& childVisible && cell != childCell) {
	...
	if (isMoveable(cell) && rect != null) {
	    ...
	    if (!hasChildren) {
		rect.setFrame(rect.getX() + dx, rect.getY() + dy,
	...
	while (it.hasNext()) {
	    ...
	    // Update Maximum
	    if (size != null)
		d.setSize(Math.max(d.getWidth(), size.getWidth()),
			Math.max(d.getHeight(), size.getHeight()));
	}
	return d;
    }
     ...
     *            the new list of points for the edge
     */
    public void setIntermediatePoints(Object edge, List<mxPoint> points) {
	List oldPoints = getPoints(edge);
	List newPoints = new ArrayList();
	newPoints.add(oldPoints.get(0));

	for (mxPoint point : points) {
	    Point2D intermed = new Point2D.Double(point.getX(), point.getY());
	    newPoints.add(intermed);
	}

	newPoints.add(oldPoints.get(oldPoints.size() - 1));
	setPoints(edge, newPoints);
	disableRouting(edge);
    }
    ...
    /**
     * Utility method to sync an mxGraph bridge to the underlying graph.
     * 
     * @param bridge
     */
    public void syncMxBridge(mxGraphBridge bridge) {
	bridge.syncMx(this.graph);
    }
