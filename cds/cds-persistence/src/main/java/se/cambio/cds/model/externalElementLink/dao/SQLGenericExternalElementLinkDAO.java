 ...
 * @author iago.corbal
 */
public class SQLGenericExternalElementLinkDAO implements GenericExternalElementLinkDAO {


	private SQLExternalElementLinkDAO dao;
	private DataSource dataSource;

	public SQLGenericExternalElementLinkDAO() throws InternalErrorException {
		dao = SQLExternalElementLinkFactory.getDAO();
		dataSource = DataSourceLocator.getDataSource(GlobalNames.CDSS_DATA_SOURCE);
	}

	public Collection<ExternalElementLinkDTO> search(String idExternalElement, String idDataSource)
	throws InternalErrorException, ModelException {
		Connection conexion = null;
		try {
			conexion = dataSource.getConnection();
			return dao.search(conexion, idExternalElement, idDataSource);
	...
	throws InternalErrorException, ModelException {
		...
		try {
			conexion = dataSource.getConnection();
			return dao.searchByElementId(conexion, idElement);
		} catch (SQLException e) {
			throw new InternalErrorException(e);
		} finally {
			GeneralOperations.closeConnection(conexion);
	...
	throws InternalErrorException, ModelException {
		...
		try {
			conexion = dataSource.getConnection();
			dao.add(conexion, externalElementLinkDTO);
		} catch (SQLException e) {
			throw new InternalErrorException(e);
		} finally {
			GeneralOperations.closeConnection(conexion);
	...
	throws InternalErrorException, ModelException {
		...
		try {
			conexion = dataSource.getConnection();
			dao.remove(conexion,externalElementLinkDTO);
		} catch (SQLException e) {
			throw new InternalErrorException(e);
		} finally {
			GeneralOperations.closeConnection(conexion);
	...
	throws InternalErrorException, ModelException {
		...
		try {
			conexion = dataSource.getConnection();
			return dao.removeByDataSourceId(conexion, dataSourceId);
		} catch (SQLException e) {
			throw new InternalErrorException(e);
		} finally {
			GeneralOperations.closeConnection(conexion);
