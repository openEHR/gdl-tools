...
public class ElementInstanceCollection {
    ...
    public void addAll(Set<ArchetypeReference> archetypeReferences, GuideManager guideManager){
     ...
     */
    public boolean matches(GeneratedArchetypeReference generatedArchetypeReference, GuideManager guideManager){
	...
	while(i.hasNext() && !matches){
	    ...
	    matches = ElementInstanceCollectionUtil.matches(generatedArchetypeReference, ar, guideManager);
	}
	return matches;
    }
	...
	Set<ElementInstance> elementInstances = new HashSet<ElementInstance>();
	for (String idArchetype : getArchetypeReferenceMap().keySet()) {
	    for (ArchetypeReference archetypeReference : getAllArchetypeReferences(idArchetype, idDomain)) {
    ...
    */
	...
	}if (Domains.EHR_ID.equals(idDomain)){
	    idAux = archetypeReference.getAggregationFunction();
	}
	if (idAux==null){
	    idAux = EMPTY_CODE;
	}
