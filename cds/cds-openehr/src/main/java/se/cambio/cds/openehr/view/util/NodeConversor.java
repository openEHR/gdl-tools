...
public class NodeConversor {
    ...
    public static int SEARCH_ONLY_LEAVES = 0;
    public static int SEARCH_ONLY_PARENT = 1;
    public static int SEARCH_ALL = 2;
	...
	return getSelectedObjects(nodoRaiz, SEARCH_ALL);
    }
    public static Collection<Object> getSelectedObjects(SelectableNode<?> nodoRaiz, int searchType){
    ...
    public static void addSelectedNodes(SelectableNode<?> nodoRaiz, Collection<SelectableNode<?>> selectedNodes, int searchType){
	Enumeration<?> e = nodoRaiz.getAllchildren();
	while (e.hasMoreElements()){
	    SelectableNode<?> node = (SelectableNode<?>)e.nextElement();
	    if (node.getSeleccionado()){
		if (searchType!=SEARCH_ONLY_LEAVES || node.isLeaf()){
		    selectedNodes.add(node);
		}
		if (searchType!=SEARCH_ONLY_PARENT){
		    addSelectedNodes(node, selectedNodes, searchType);
		}
	    }else if (node.getContineneSeleccionado()){
		if (searchType!=SEARCH_ONLY_PARENT || !node.isLeaf()){
		    addSelectedNodes(node, selectedNodes, searchType);
		}
	    }
	}
